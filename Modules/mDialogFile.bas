...
Public Function GetFileName( _
 ...
 Dim i    As Long
    ...
    If (lRet) Then
        GetFileName = wgTrimNull(uOFN.lpstrFile)
    End If
End Function
...
Private Function wgTrimNull(StartString As String) As String
  
  Dim lPos As Long
  
    lPos = InStr(StartString, vbNullChar)
    If (lPos) Then
        wgTrimNull = Left$(StartString, lPos - 1)
      Else
        wgTrimNull = StartString
    End If
End Function
